...
int main() {
	...
	while(true){
		Mem_Memset(g_wpath,0,sizeof(g_wpath));
		...
		int ret = File_FindFirst(reinterpret_cast<const char_const16_t*>(g_wpath), &findHandle, reinterpret_cast<unsigned short*>(fileName), &findInfoBuf);
		while (ret>=0 && dirFiles<maxFiles ){
			...
			//serch the next
			ret = File_FindNext(findHandle, reinterpret_cast<unsigned short*>(fileName), &findInfoBuf);
		}
		...
		if (go>=0 && go<dirFiles){
			...
			if(thisfile.type=='D'){ //type = folder
				...
				Mem_Memset(thisFileName,0,sizeof(thisFileName));
...
void* realloc_sized(int oldptr, size_t oldsz, size_t newsz){
	void* newptr = malloc(newsz);
	Mem_Memset(newptr,oldptr,oldsz);
	std::free(reinterpret_cast<void*>(oldptr));
	return newptr;
}
...
int  stbi_my_read(void *user,char *data,int size){
	int ret = (int) File_Read((int)(intptr_t)user, data, size);
	return ret;
}
void stbi_my_skip(void *user,int n){
	FILE* f = (FILE*)user; 
...
void showImage(char* fileName){
	...
	stbi__context s;
	stbi__start_callbacks(&s, &stbi__my_callbacks, (int)fp);
...
void showHex(char* fileName){
	...
	uint8_t* addr;
	fp = File_Open(fileName, FILE_OPEN_READ);
	auto _ = File_GetAddr(fp,0,(const void**)&addr);
	File_Close(fp);
...
void playWav(char* fileName){
    ...
    // open file
	unsigned char* wav;
	int fp = File_Open(fileName,FILE_OPEN_READ);
	...
	Serial_Open(reinterpret_cast<const struct Serial_Settings*>(mode));
	
	//send audio
	uint16_t i = 0x2c; //skip the first 44 bytes (the wav header and assume 8bit mono 11kHz
	int j=0;
	while(true){
		if(File_GetAddr(fp,j*4096,(const void**)&wav)){ //get the address of the next 4k block
